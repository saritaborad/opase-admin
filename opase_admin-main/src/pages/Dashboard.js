import React, { useEffect, useState } from "react";
import UserLayout from "../components/UserLayout";
import Dash_1 from "../images/dash-t-1.svg";
import Dash_2 from "../images/dash-t-2.svg";
import Dash_3 from "../images/dash-t-3.svg";
import Dash_4 from "../images/dash-t-4.svg";
import Dash_5 from "../images/dash-t-5.svg";
import Chart from "react-apexcharts";
import { Link, useNavigate } from "react-router-dom";
import { ToastContainer, toast } from "react-toastify";
import "react-toastify/dist/ReactToastify.css";
import RtdDatatable from "../components/DataTable/RtdDatatable";
import { PostApi } from "../api/api-service";
import { API_Path } from "../const";
import Moment from "react-moment";

let newArrX = [];
let newArrY = [];
let donutData = [];
let newArr1X = [];
let newArr1Y = [];
let color1 = [];

export default function Dashboard(params) {
  const navigate = useNavigate();

  const [dashboardData, setDashboardData] = useState({});
  const [cashoutData, setCashoutData] = useState([]);
  const [donutChart, setDonutchart] = useState([]);
  // const [graphXData, setGraphXData] = useState([]);
  // const [graphYData, setGraphYData] = useState([]);
  const [graphCData, setGraphCData] = useState([]);
  const [graphCYData, setGraphCYData] = useState([]);
  const [transation, setTransaction] = useState([]);
  const [loaded, setLoaded] = useState(false);
  const [option, set_option] = useState({
    sizePerPage: 10,
    search: "",
    totalRecord: 0,
    page: 0,
    sort: "id",
    order: "DESC",
  });

  const handle_click = (name) => {
    navigate(name);
  };

  useEffect(() => {
    GetDashBoardData();
    getCashoutdetails();
    getTransactionData();
    getRevenueChartData("today");
  }, []);

  const GetDashBoardData = () => {
    const addDashboardDetails = new Promise((resolve) => {
      resolve(PostApi(API_Path.getDashboardData));
    });
    addDashboardDetails.then((res) => {
      if (res.status === 200) {
        setLoaded(true);
        donutData = [];
        donutData.push(res.data.data?.requestGraph?.chat);
        donutData.push(res.data.data?.requestGraph?.videos);
        donutData.push(res.data.data?.requestGraph?.videos24);
        setDonutchart(donutData);
        setDashboardData(res.data.data);
      } else {
        toast.error(res.data.message);
      }
    });
  };

  const getRevenueChartData = (type) => {
    let arr = [];
    const data = { graph: type };
    const getRevenueChartData = new Promise((resolve) => {
      resolve(PostApi(API_Path.GetRevenueChart, data));
    });
    getRevenueChartData.then((res) => {
      if (res.status === 200) {
        res.data.data.map((item) => {
          let obj = {};
          obj.x = item.y;
          obj.y = item.x;
          arr.push(obj);
          newArr1X.push(item.x);
          if (!newArr1Y.includes(item.y)) {
            newArr1Y.push(item.y);
          }
          color1.push("#fff");
        });

        setGraphCData(arr);
        setGraphCYData(newArr1Y);
      } else {
        toast.error(res.data.message);
      }
    });
  };

  const getCashoutdetails = () => {
    const addCashoutDetails = new Promise((resolve) => {
      resolve(PostApi(API_Path.GEtTransferData));
    });
    addCashoutDetails.then((res) => {
      if (res.status === 200) {
        if (res.data.data.length > 0) {
          setCashoutData(res.data.data.slice(0, 6));
        }
      } else {
        // toast.error(res.data.message);
      }
    });
  };

  const getTransactionData = () => {
    const getTransactionData = new Promise((resolve) => {
      resolve(PostApi(API_Path.refundReward));
    });
    getTransactionData.then((res) => {
      if (res.status === 200) {
        if (res.data.data.refundReq.length > 0) {
          setTransaction(res.data.data.refundReq.slice(0, 6));
        }
      } else {
        // toast.error(res.data.message);
      }
    });
  };
  // ================= Chart-1 ================= //
  const chart1 = {
    series: donutChart,
    chart: {
      type: "donut",
      height: 300,
      fontFamily: "Roboto",
      color: "#fff",
      zoom: {
        enabled: true,
      },
    },
    stroke: {
      show: true,
      curve: "smooth",
      lineCap: "butt",
      width: 0,
      dashArray: 0,
    },
    dataLabels: {
      enabled: true,
      formatter: function (val) {
        return val?.toFixed(2) + "%";
      },
    },
    legend: {
      show: true,
      position: "bottom",
      floating: false,
      verticalAlign: "bottom",
      fontSize: "16px",
      fontFamily: "Roboto",
      fontWeight: 500,
      labels: {
        colors: "#fff",
      },

      markers: {
        radius: 10,
        width: 12,
        height: 12,
        strokeWidth: 1,
        strokeColor: ["#7ED957", "#FFB946", "#EB5757"],
        fillColors: "000",
      },

      itemMargin: {
        horizontal: 20,
        vertical: 0,
      },
    },
    plotOptions: {
      pie: {
        donut: {
          labels: {
            show: true,
            total: {
              show: false,
              color: "#fff",
              fontSize: "24px",
            },
          },
          size: "65%",
        },
      },
    },
    tooltip: {
      y: {
        formatter: function (val) {
          return val + "%";
        },
      },
    },
    labels: ["Chats", "Videos", "Videos (24 hours delivery)"],
    colors: ["#7ED957", "#FFB946", "#EB5757"],
    responsive: [
      {
        breakpoint: 480,
        options: {
          chart: {
            width: "100%",
          },
          legend: {
            position: "bottom",
            color: "#fff",
          },
        },
      },
    ],
  };

  // ================= Chart-2 ================= //

  const chart2 = {
    series: [
      {
        name: "Clients Overview",
        data: graphCData,
      },
    ],
    fill: {
      opacity: 1,
    },
    chart: {
      height: 300,
      type: "area",
      fontFamily: "DM Sans",
      zoom: {
        enabled: true,
      },
      toolbar: {
        show: false,
      },
      tooltip: {
        background: "#000",
      },
    },
    dataLabels: {
      enabled: false,
    },
    colors: ["#684BFF"],
    stroke: {
      show: true,
      curve: "smooth",
      lineCap: "butt",
      width: 2,
      dashArray: 0,
    },
    xaxis: {
      categories: graphCYData,
      axisBorder: { show: false },

      axisTicks: {
        show: false,
      },

      labels: {
        rotate: -60,
        style: {
          fontSize: "12px",
          fontWeight: "400",
          colors: "#fff",
          fontFamily: "roboto",
        },
      },
    },
    grid: {
      show: true,
      borderColor: "rgba(35, 34, 34, .1)",
      strokeDashArray: 3,
      position: "back",
      yaxis: {
        lines: {
          show: true,
        },
      },
    },
    yaxis: {
      tooltip: {
        y: {
          formatter: function (val) {
            return val;
          },
        },
      },
      labels: {
        style: {
          colors: "#fff",
          fontSize: "12px",
          fontWeight: "400",
          fontFamily: "Roboto",
        },
        formatter: function (value) {
          return value + "K";
        },
      },
    },
    responsive: [
      {
        breakpoint: 576,
        options: {
          xaxis: {
            labels: {
              rotate: -80,
              style: {
                fontSize: "9px",
              },
            },
          },
          yaxis: {
            labels: {
              style: {
                fontSize: "9px",
              },
            },
          },
        },
      },
    ],
  };
  //  ============== TABLE 1 ================ //

  const columns1 = [
    {
      value: "invoice",
      label: "Invoice Id",
      options: {
        filter: false,
        sort: false,
        customBodyRender: (data, i) => {
          return <span>#{data[i].invoice}</span>;
        },
      },
    },
    {
      value: "User_Name",
      label: "Username",
      options: {
        filter: false,
        sort: false,
      },
    },
    {
      value: "Date",
      label: "Date",
      options: {
        filter: false,
        sort: false,
        customBodyRender: (data, i) => {
          return <Moment format="DD/MM/YYYY">{data[i].created_at}</Moment>;
        },
      },
    },
    {
      value: "amount",
      label: "Amount",
      options: {
        filter: false,
        sort: false,
        customBodyRender: (data, i) => {
          return <span>${String(data[i]?.amount)?.replace(/(?<!\..*)(\d)(?=(?:\d{3})+(?:\.|$))/g, "$1,")}</span>;
        },
      },
    },
  ];

  //  ============== TABLE 2 ================ //

  const columns2 = [
    {
      value: "invoice",
      label: "Invoice Id",
      options: {
        filter: false,
        sort: false,
        customBodyRender: (data, i) => {
          return <span>#{data[i].invoice}</span>;
        },
      },
    },
    {
      value: "fullname",
      label: "Username",
      options: {
        filter: false,
        sort: false,
      },
    },
    {
      value: "Type",
      label: "Type",
      options: {
        filter: false,
        sort: false,
        customBodyRender: (data, i) => {
          return <div>{data[i].payment_type === 1 ? "Stripe" : data[i].payment_type === 2 ? "Stripe to wallet" : data[i].payment_type === 3 ? "Wallet" : data[i].payment_type === 4 ? "Google Pay" : data[i].payment_type === 5 ? "Google to wallet" : data[i].payment_type === 6 ? "Apple pay" : data[i].payment_type === 7 ? "Apple to wallet" : data[i].payment_type === 8 && "Transfer to talent"}</div>;
        },
      },
    },
    {
      value: "From",
      label: "From",
      options: {
        filter: false,
        sort: false,
        customBodyRender: (data, i) => {
          return <div>{data[i].chat_request === 1 ? "Chat" : "Video"}</div>;
        },
      },
    },
    {
      value: "Date",
      label: "Date",
      options: {
        filter: false,
        sort: false,
        customBodyRender: (data, i) => {
          return <Moment format="DD/MM/YYYY">{data[i].updated_at}</Moment>;
        },
      },
    },
    {
      value: "amount",
      label: "Amount",
      options: {
        filter: false,
        sort: false,
        customBodyRender: (data, i) => {
          return <span>${String(data[i]?.amount)?.replace(/(?<!\..*)(\d)(?=(?:\d{3})+(?:\.|$))/g, "$1,")}</span>;
        },
      },
    },
  ];

  const handleChangeData = (e) => {
    setGraphCData([]);
    newArr1X = [];
    setGraphCYData([]);
    newArr1Y = [];
    getRevenueChartData(e.target.value);
  };

  const tableCallBack = (option) => {
    set_option(option);
  };

  const formatNum = (num) => {
    return String(num)?.replace(/(?<!\..*)(\d)(?=(?:\d{3})+(?:\.|$))/g, "$1,");
  };
  return (
    <>
      <UserLayout>
        <div className="content-main-section">
          <div className="container-fluid">
            <div className="row">
              <div className="col-12 mt-3">
                <div className="comn-title-main d-block  d-sm-flex justify-content-between">
                  <h1 className="mb-0">Dashboard</h1>
                </div>
              </div>
              <div className="col-12 mt-3">
                <div className="row me-0 justify-content-center">
                  <div className="col-xxl col-xl-3 col-md-4 col-6 pe-0 mb-3">
                    <div className="dash-top-box fix-span">
                      <span className="dash-span-3">
                        <img src={Dash_3} alt="3" />
                      </span>
                      <p>{loaded && "$ " + formatNum(dashboardData?.totalRevenue)}</p>
                      <div className="dash-top-box-info d-flex align-items-center">
                        <bdi>Total revenue</bdi>
                      </div>
                    </div>
                  </div>
                  <div className="col-xxl col-xl-3 col-md-4 col-6 pe-0 mb-3">
                    <div className="dash-top-box fix-span">
                      <span className="dash-span-4">
                        <img src={Dash_4} alt="4" />
                      </span>
                      <p>{loaded && "$ " + formatNum(dashboardData.chatRevenue ? dashboardData.chatRevenue : 0)}</p>
                      <div className="dash-top-box-info d-flex align-items-center">
                        <bdi>Chat revenue</bdi>
                      </div>
                    </div>
                  </div>
                  <div className="col-xxl col-xl-3 col-md-4 col-6 pe-0 mb-3">
                    <div className="dash-top-box fix-span">
                      <span className="dash-span-5">
                        <img src={Dash_5} alt="5" />
                      </span>
                      <p>{loaded && "$ " + formatNum(dashboardData.videoRevenue)}</p>
                      <div className="dash-top-box-info d-flex align-items-center">
                        <bdi>Video revenue</bdi>
                      </div>
                    </div>
                  </div>
                  <div className="col-xxl col-xl-3 col-md-4 col-6 pe-0 mb-3">
                    <div className="dash-top-box fix-span">
                      <div className="d-flex align-items-top ">
                        <span className="dash-span-1">
                          <img src={Dash_1} alt="1" />
                        </span>
                        <svg width="20" height="21" viewBox="0 0 20 21" fill="none" xmlns="http://www.w3.org/2000/svg" className="ms-auto" onClick={() => handle_click("/requests")}>
                          <path d="M19 12.3812V17.6193C19 18.175 18.7893 18.708 18.4142 19.1009C18.0391 19.4938 17.5304 19.7146 17 19.7146H3C2.46957 19.7146 1.96086 19.4938 1.58579 19.1009C1.21071 18.708 1 18.175 1 17.6193V2.95266C1 2.39697 1.21071 1.86404 1.58579 1.4711C1.96086 1.07817 2.46957 0.857422 3 0.857422H8M11.5 8.71457L19 0.857422L11.5 8.71457ZM14 0.857422H19V6.09552L14 0.857422Z" stroke="#F44C73" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round" />
                        </svg>
                      </div>
                      <p>{loaded && formatNum(dashboardData.Request)}</p>
                      <div className="dash-top-box-info d-flex align-items-center">
                        <bdi>Requests</bdi>
                      </div>
                    </div>
                  </div>
                  <div className="col-xxl col-xl-3 col-md-4 col-6 pe-0 mb-3">
                    <div className="dash-top-box fix-span">
                      <div className="d-flex align-items-top ">
                        <span className="dash-span-2">
                          <img src={Dash_2} alt="2" />
                        </span>
                        <svg width="20" height="21" viewBox="0 0 20 21" fill="none" xmlns="http://www.w3.org/2000/svg" className="ms-auto" onClick={() => handle_click("/user")}>
                          <path d="M19 12.3812V17.6193C19 18.175 18.7893 18.708 18.4142 19.1009C18.0391 19.4938 17.5304 19.7146 17 19.7146H3C2.46957 19.7146 1.96086 19.4938 1.58579 19.1009C1.21071 18.708 1 18.175 1 17.6193V2.95266C1 2.39697 1.21071 1.86404 1.58579 1.4711C1.96086 1.07817 2.46957 0.857422 3 0.857422H8M11.5 8.71457L19 0.857422L11.5 8.71457ZM14 0.857422H19V6.09552L14 0.857422Z" stroke="#F44C73" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round" />
                        </svg>
                      </div>
                      <p>{loaded && formatNum(dashboardData.visiter)}</p>
                      <div className="dash-top-box-info d-flex align-items-center">
                        <bdi>Visiters</bdi>
                      </div>
                    </div>
                  </div>
                </div>
              </div>
              <div className="col-12">
                <div className="row me-0">
                  <div className="col-xxl-4 mb-3  ">
                    <div className="dash-part-hdr">
                      <div className="dash-part-hdr-top d-flex">
                        <span>Requests</span>
                        <span className="ms-auto chart-data">{loaded && formatNum(dashboardData?.requestGraph?.chat + dashboardData?.requestGraph?.videos + dashboardData?.requestGraph?.videos24)}</span>
                      </div>
                      <div className="chart-main-part">
                        <Chart options={chart1} series={chart1.series} height={300} type="donut" />
                      </div>
                    </div>
                  </div>
                  <div className="col-xxl-8 mb-3 pe-0">
                    <div className="dash-part-hdr">
                      <div className="dash-part-hdr-top d-flex">
                        <span>Revenue</span>
                        <div className="ms-auto chart-data">
                          <select className="form-select w-100" name="all" onChange={handleChangeData}>
                            {/* <option className="cmn-online-payment" value="0">Last: 1 Month</option> */}
                            <option className="cmn-online-payment text-center" value="today">
                              Today
                            </option>
                            <option className="cmn-online-payment text-center" value="yestrday">
                              Yesterday
                            </option>
                            <option className="cmn-online-payment text-center" value="weekly">
                              Weekly
                            </option>
                            <option className="cmn-online-payment text-center" value="monthly">
                              Monthly
                            </option>
                            <option className="cmn-online-payment text-center" value="lastYear">
                              Last year
                            </option>
                          </select>
                        </div>
                      </div>
                      <div className="chart-main-part" id="responsive-chart">
                        <Chart options={chart2} series={chart2.series} type="area" height={300} />
                      </div>
                    </div>
                  </div>
                </div>
              </div>
              <div className="col-12">
                <div className="row ">
                  <div className="col-xxl-5 mb-3">
                    <div className="dash-part-hdr">
                      <div className="dash-part-table-hdr d-flex align-items-center">
                        <span>Cashouts (Talents)</span>
                      </div>
                      <div className="col-12 ">
                        <div className="comn-table-black-bg">
                          <div className="">
                            <RtdDatatable option={option} columns={columns1} data={cashoutData} tableCallBack={tableCallBack} />
                          </div>
                        </div>
                      </div>
                    </div>
                  </div>
                  <div className="col-xxl-7 mb-3">
                    <div className="dash-part-hdr">
                      <div className="dash-part-table-hdr d-flex align-items-center justify-content-between">
                        <span>Last transations (Users)</span>
                        <Link to="/refund">
                          <bdi className="ms-auto table-view-link">View all</bdi>
                        </Link>
                      </div>
                      <div className="comn-table-black-bg">
                        <div className="">
                          <RtdDatatable option={option} columns={columns2} data={transation} tableCallBack={tableCallBack} />
                        </div>
                      </div>
                    </div>
                  </div>
                </div>
              </div>
            </div>
          </div>
        </div>
      </UserLayout>
      <ToastContainer autoClose={1000} />;
    </>
  );
}
