...
export default function HomePageSection(params) {
  const navigate = useNavigate();

  const homeBannerData = (id) => {
    navigate(`/edit-home-page-section`, {
      state: { id: id },
    });
  };
  ...
  const columns = [
    {
      ...
      options: {
        ...
        customBodyRender: (data, i) => {
          return <div className="table-user-div d-flex align-items-center">{data[i].position == 1 ? <img src={data[i].img_url} alt="image" className="img-fluid" /> : data[i].position == 2 ? <video src={data[i].video_url} alt="video"></video> : <img src={User_Img} alt="product" className="img-fluid" width="50px" />}</div>;
        },
      },
    },
    {
  ...
  let getHomePageData = () => {
    let GetWebsiteHomePageSectionData = new Promise((resolve) => {
      resolve(GetApi(API_Path.getWebsiteHomePage));
    });
