...
import OpaseProfile from "../images/hdr profile img.png";
...
export default function Header(params) {
  ...
  const [notifStatus, setNotifStatus] = useState(1);
  const [notificationData, setNotificationData] = useState(1);
  const [talentData, setTalentData] = useState(1);
  ...
  const talentDetailsData = (id) => {
    ...
    // window.location.href = `/talent-details/${id}`;
  };
  ...
  const getAllIProfile = () => {
    let data = { option: option, single: 1 };
  ...
  const searchTalent = (search_val) => {
    let data = { search: search_val };
    const searchTalentData = new Promise((resolve) => {
      resolve(PostApi(API_Path.searchTalent, data));
    });
    searchTalentData.then((res) => {
      if (res.status === 200) {
        setTalentData(res.data.data);
      }
    });
  };
  ...
  const handleNotification = (e) => {
    ...
    if (document.getElementById(`offer-status`)) {
      document.getElementById(`offer-status`).checked = true;
    }
  };
  const handleLogout = () => {
    ...
    <>
      <header className="header-top-section">
        <nav className="navbar fixed-top">
          <ul className="d-flex align-items-senter me-auto hdr-top-box-inr">
            <li>
              <div className="hdr-logo-top d-xl-none ms-xl-0 ms-4 text-xl-center d-flex align-items-center">
                <Link to="/dashboard">
                  <img src={OpaseLogo} className="ms-4" alt="Opase" />
            ...
            <li className="position-relative">
              <bdi className=" d-md-block  d-none ms-5 ms-xl-0 position-relative">
                <input type="search" className="form-control login-comn-input searchbar  ps-3" placeholder="Search Anything" onChange={handlesearch} onBlur={handleSearchBlurIn} onKeyDown={handlesearch} />
              ...
              <div className="search-result-box mt-2" id="search-wrapper" onClick={handleSearchBlur}>
                <div className="related-serch-main">
                  <div className="search-his-body-scroll">
                    <ul>
                      {talentData && talentData.length > 0 ? (
                        talentData?.map((item, i) => {
                          return (
                            <li
                              onClick={() => {
                                talentDetailsData(item.id);
                                setTalentId(item.id);
                              }}
                            >
                              <div>
                                <span>{item.fullname}</span>
                                <p className="cursor-pointer">{item.username}</p>
                              </div>
          ...
          <ul className="d-flex align-items-center hdr-rgt-part responsive-view-menu" id="user-detail">
            ...
            <li className="profile-hdr-drop-class ps-3">
              <Dropdown className="d-inline" drop="left">
                <Dropdown.Toggle variant="transparent" id="dropdown-autoclose-true">
                  <div className="profile-hdr-drop ms-1">
                    <div className="profile-pic me-1">
                      <img src={user !== null ? user?.image : OpaseProfile} alt="profile" />
                    </div>
                    <div className="profil-detail-section d-flex text-start ms-2">
                      <div>
                        <span>{user ? user?.user_name : userName}</span>
                        {/* <p>Talent</p> */}
                      </div>
                      <div>
                ...
                <Dropdown.Menu>
                  {/* <Dropdown.Item className="d-flex align-items-center position-relative">
                    <div className="ms-2">
                      Notifications */}
                  {/* <div className="custm-toggel-switch">
                        <div className="form-check form-switch">
                          <input className="form-check-input mt-0" type="checkbox" id="offer-status" onChange={handleNotification} />
                        </div>
                      </div>
                    </div>
                    <div className="custm-toggel-switch ms-auto me-2">
                      <input type="checkbox" hidden="hidden" />
                      <label className="switch" for="username"></label> */}
                  {/* <div className="custm-toggel-switch">
                        <div className="form-check form-switch">
                          <input className="form-check-input mt-0" type="checkbox" id="offer-status" onChange={handleNotification} defaultChecked />
                        </div>
                      </div> */}
                  {/* </div>
                  </Dropdown.Item> */}
                  <Dropdown.Item onClick={() => handleLogout()} className="d-flex align-items-center mt-3">
      ...
      {/* <Modal show={logoutModel} onHide={() => setLogoutModel(false)} size="sm" className="comn-modal-style" aria-labelledby="contained-modal-title-vcenter" centered>
        <Modal.Header className="p-4 pb-0 ">
          <h4 className="text-center w-100">Logout</h4>
        </Modal.Header>
